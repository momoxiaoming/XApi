...
import com.xm.api.R
import com.xm.xapi.ktx.gone
import com.xm.xapi.ktx.isNetConnected
import com.xm.xapi.ktx.show


/**
 *  状态layout, 包含三种状态切换, 加载中, 加载empty,加载失败
 *
 * @author mmxm
...
class StateLayout @JvmOverloads constructor(
    ...
    private var onErrorBlock: (View.(data: Any?) -> Unit)? = XStateManager.onErrorBlock
    private var onEmptyBlock: (View.(data: Any?) -> Unit)? = XStateManager.onEmptyBlock
    private var onLoadingBlock: (View.(data: Any?) -> Unit)? = XStateManager.onLoadingBlock
    ...
    fun showLoading(data: Any? = null, refresh: Boolean = true): StateLayout {
        ...
        if (loadingLayout == NO_ID) {
            //使用全局参数
            loadingLayout = XStateManager.loadingLayout
        }
    ...
    fun showError(any: Any? = null, autoRetryLoading: Boolean = true) {
        ...
        if (errorLayout == NO_ID) {
            //使用全局参数
            errorLayout = XStateManager.errorLayout
        }
    ...
    fun showEmpty(data: Any? = null, autoRetryLoading: Boolean = true) {
        ...
        if (emptyLayout == NO_ID) {
            //使用全局参数
            emptyLayout = XStateManager.emptyLayout
        }
        this.autoRetryLoading = autoRetryLoading
        if (emptyLayout != NO_ID) {
        ...
        runMain {
            ...
            when (layoutId) {
                emptyLayout -> { //空
                    ...
                    if (autoRetryLoading) {
                        if (emptyRetryIds.isEmpty()) emptyRetryIds = XStateManager.emptyRetryIds
                        emptyRetryIds.forEach {
                            findViewById<View>(it).tingleClick {
                                //重试
                                showLoading(data)
                            }
                    ...
                    onEmptyBlock?.invoke(view, data)
                }

                loadingLayout -> { //加载
                    status = StateStatus.LOADING
                    onLoadingBlock?.invoke(view, data)
                ...
                errorLayout -> { //错误
                    ...
                    if (autoRetryLoading) {
                        if (errorRetryIds.isEmpty()) errorRetryIds = XStateManager.errorRetryIds
